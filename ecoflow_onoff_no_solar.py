...
def set_ac_charging(serial_number, enable: bool, max_watts: int = None):
    ...
    params = {
        "fastChgWatts":  chg_watts,     # ← was chgWatts
        "slowChgWatts":  chg_watts,
...
# ── REST helpers (signing unchanged — only used to get MQTT creds) ────────────
def _sign_get(prefix: str) -> dict:
    ...
    timestamp = str(int(time.time() * 1000))
    msg       = prefix + f"accessKey={ECOFLOW_ACCESS_KEY}&nonce={nonce}&timestamp={timestamp}"
    sig       = hmac.new(ECOFLOW_SECRET_KEY.encode(), msg.encode(), hashlib.sha256).hexdigest()
    return {
        "accessKey": ECOFLOW_ACCESS_KEY,
        "nonce":     nonce,
        "timestamp": timestamp,
...
def publish_mqtt_command(sn: str, payload_dict: dict):
    ...
    def on_connect(client, userdata, flags, reason_code, properties):  # 5 args for V2
        if reason_code == 0:
            client.publish(command_topic, payload, qos=1)
        else:
            result["error"] = f"MQTT connect failed: rc={reason_code}"
            client.disconnect()

    def on_publish(client, userdata, mid, reason_code=None, properties=None):  # V2 passes extra args
        result["success"] = True
        client.disconnect()
...
def disable_backup_reserve(sn: str, current_reserve_pct: int = 25):
    """
    Turn off Backup Reserve (isConfig: 0).
    This allows AC charging regardless of current SOC.
    Pass the current bpPowerSoc value so it isn't reset.
    """
    return publish_mqtt_command(sn, {
        "id":          str(int(time.time() * 1000)),
        "version":     "1.0",
        "sn":          sn,
        "moduleType":  1,
        ...
        "params": {
            "isConfig":   0,                  # 0 = Backup Reserve OFF
            "bpPowerSoc": current_reserve_pct, # preserve existing value
            "minDsgSoc":  0,
            "minChgSoc":  0,
        },
    })

def enable_backup_reserve(sn: str, reserve_pct: int = 25):
    """Turn Backup Reserve back on."""
    return publish_mqtt_command(sn, {
        "id":          str(int(time.time() * 1000)),
        "version":     "1.0",
        "sn":          sn,
        ...
        "operateType": "watthConfig",
        "params": {
            "isConfig":   1,          # 1 = Backup Reserve ON
            "bpPowerSoc": reserve_pct,
            "minDsgSoc":  0,
            "minChgSoc":  0,
        },
    })
...
#Enable AC charging if export power is negative (i.e. we're exporting to the grid).
print("Enabling AC charging...")
response = set_ac_charging_mqtt(delta2_sn, True, 500)
print(f"Response from Ecoflow API: {response}")
response = disable_backup_reserve(delta2_sn, current_reserve_pct=25)
print(f"Response from Ecoflow API: {response}")


"""
# Disable AC charging if export power is positive (i.e. we're importing from the grid).
print("Disabling AC charging...")
response = set_ac_charging_mqtt(delta2_sn, False, 300)
print(f"Response from Ecoflow API: {response}")
response = enable_backup_reserve(delta2_sn, reserve_pct=20)
print(f"Response from Ecoflow API: {response}")
"""
...
if export_power < 0:
    ...
    response = set_ac_charging_mqtt(delta2_sn, False)
    print(f"Response from Ecoflow API: {response}")
"""
